...
USERS_DB = {
    "admin": {
        "username": "admin",
        "email": "admin@nigraniai.com",
        "password": "admin123",
        "role": "admin"
    },
    "operator": {
        "username": "operator",
        "email": "operator@nigraniai.com", 
...
class NigraniAIHandler(BaseHTTPRequestHandler):
    def do_OPTIONS(self):
    ...
    def do_DELETE(self):
        ...
        if path == '/alerts':
            ...
            if user:
                count = len(ALERTS)
                ALERTS.clear()
                print(f"🗑️ All alerts cleared by {user['username']} ({count} alerts)")
...
def main():
    """Start the simple backend server"""
    print("🛡️ NigraniAI Simple Backend Server")
    print("=" * 50)
    ...
    try:
        server_address = ('', 8000)
        httpd = HTTPServer(server_address, NigraniAIHandler)
